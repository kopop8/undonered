<template>
  <q-page class="q-ma-lg">
    <q-header bordered class="bg-secondary text-white">
      <q-toolbar>
        <q-toolbar-title class="text-center">
          Review
        </q-toolbar-title>
      </q-toolbar>
    </q-header>
    <q-card class="q-ma-sm">
      <q-item>
        <q-item-section avatar>
          <q-avatar @click="openDialog = !openDialog" v-if="img" style="height: 100px; width: 100px">
            <q-img :src="img"></q-img>
          </q-avatar>
          <q-avatar @click="getPicture" color="secondary text-white" style="height: 100px; width: 100px" v-else>
            <q-icon name="camera_alt"/>
          </q-avatar>
        </q-item-section>
        <q-item-section>
          <q-input filled  stack-label label="Toughts?" v-model="description"  data-cy="review-input"
                   placeholder="verynice!"></q-input>
        </q-item-section>
      </q-item>
    </q-card>
    <span class="q-ma-sm text-grey-8 text-bold">Rating</span>
    <q-separator class="q-ma-sm" inset/>
    <q-rating size="45px" v-model="rating" :max="5"/>
    <q-separator class="q-ma-sm" inset/>
    <q-card>
      <q-card-section>
        <q-select stack-label          option-value="value"
                  option-label="label"   map-options emit-value v-model="selectedOption" :options="restaurantOptions" filled label="Select a restaurant"/>
      </q-card-section>
    </q-card>
    <q-btn class="float-right q-ma-sm" label="Post"  data-cy="review-post" @click="postReview" no-caps push color="primary"></q-btn>
    <q-dialog v-model="openDialog">
      <q-img style="min-width: 80vw; width: 80vw;" :src="img">
        <div class="absolute-bottom">
          <q-btn @click="getPicture" push color="secondary" icon="refresh"></q-btn>
          <q-btn class="float-right" push color="primary" v-close-popup @click="openDialog = false" icon="check"/>
        </div>
      </q-img>
    </q-dialog>
  </q-page>
</template>

<script>
export default {
  name: 'PageReview',
  data () {
    return {
      rating: 3,
      selectedRestaurant: null,
      openDialog: false,
      lat: 0,
      long: 0,
      img: null,
      selectedOption: '0',
      description: ''
    }
  },
  created () {
    // open camera app
    this.getPicture()
    // get the location of your gps and the nearby restaurants
    this.getLocationAndRestaurants()
  },
  computed: {
    account () {
      return this.$store.getters['account/ACCOUNT']
    },
    restaurants () {
      return this.$store.getters['restaurants/RESTAURANTS']
    },
    // make a options arraylist of all gotten restaurants
    restaurantOptions () {
      let options = []
      for (let i in this.restaurants) {
        options.push({ label: `${this.restaurants[i].title} (${(this.restaurants[i].distance / 1000).toFixed(2)} km)`, value: i, restaurant: this.restaurants[i] })
      }
      return options
    }
  },
  watch: {
    // watch if rating is not null othewise set it to 1
    rating () {
      if (this.rating === 0) {
        this.rating = 1
      }
    }
  },
  methods: {
    async getLocationAndRestaurants () {
      // if needed ask for permission
      if (navigator.geolocation) {
        // get current location
        navigator.geolocation.getCurrentPosition((position) => {
          this.lat = position.coords.latitude
          this.long = position.coords.longitude
          if (this.lat !== 0 && this.long !== 0) {
            let options = {
              lat: this.lat,
              long: this.long,
              radius: 9000
            }
            // get nearby restaurants 9km
            this.$store.dispatch('restaurants/getRestaurants', options).then(response => {
              if (response.data.length === 0) {
                this.$q.notify('no restaurants found in your area')
              }
            })
          }
        }, (err) => {
          // console.error('err', err)
          this.getRestaurantsBoundingBox()
          this.$q.notify({ message: `Error occured : using gps ${err}`, color: 'red' })
        })
      } else {
        // no acces given to gps or has no gps
        this.getRestaurantsBoundingBox()
        this.$q.notify({ message: 'No access to gps, give access in settings and restart app', color: 'red' })
      }
    },
    async getRestaurantsBoundingBox () {
      // if (this.lat !== 0 && this.long !== 0) {
      // console.log('getting rest with bounding box')
      await this.$store.dispatch('restaurants/getRestaurantsBoundingBox')
      // }
    },
    postReview () {
      // this is for testing
      // if testing on computer there is no camera acces
      if (this.restaurants.length === 0 || !this.restaurants) {
        this.$q.notify('Cant post because there are no restaurants nearby')
      } else { // const review = {
        //   restaurant: this.restaurants[this.selectedOption],
        //   description: this.description,
        //   rating: this.rating,
        //   img: this.img,
        //   firstName: this.account.firstName,
        //   lastName: this.account.lastName
        // }
        const url = '/reviews'
        const body = {
          restaurantId: this.restaurants[this.selectedOption].id,
          description: this.description,
          rating: this.rating,
          image: this.img
        }
        this.$axios.post(url, body).then(response => {
          let options = {
            lat: this.lat,
            long: this.long,
            radius: this.$store.getters['account/RADIUS'] * 1000
          }
          let review = response.data
          this.$store.dispatch('restaurants/getRestaurants', options)
          this.$router.push({ name: 'index', params: { review: review } })
        }).catch(err => {
          this.$q.notify(err.message)
        })
      }
    },
    // open camera
    getPicture () {
      // console.log('getPicture')
      if (navigator.camera) {
        navigator.camera.getPicture(
          data => { // on success
            this.img = `data:image/jpeg;base64,${data}`
            // this.decreaseBase64(this.img)
          },
          () => { // on fail
            // this.$q.notify('Could not access device camera.')
          },
          {
            // camera options
            destinationType: navigator.camera.DestinationType.DATA_URL
          }
        )
      } else {
        // set a placeholder image if there is no access to the camera
        // this.img = 'data:image/png;base64,'
        this.img = 'data:image/jpeg;base64,'
        // this.decreaseBase64(this.img)
        this.$q.notify('Cant find camera')
      }
    },
    // not working yet
    decreaseBase64 (img) {
      let canvas = document.createElement('canvas')
      canvas.width = img.width
      canvas.height = img.height
      let ctx = canvas.getContext('2d')
      ctx.scale(0.05, 0.05)
      ctx.drawImage(img)
      let base64 = canvas.toDataURL('image/jpeg')
      // console.log(base64)
      return base64
    }
  }
}
</script>
